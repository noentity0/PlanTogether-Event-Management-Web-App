...
function ExplorePage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      fetchEvents(
        { search: search.trim(), category, includePast: false },
    ...
    <div className="space-y-8">
      <section className="glass-panel rounded-[2.5rem] p-8 sm:p-10">
        ...
        <p className="mt-4 max-w-2xl text-sm leading-7 text-textmuted">
          Search by title, description, or location, then filter by category to find upcoming events faster.
        </p>
      ...
      {eventsLoading ? (
        <div className="glass-panel rounded-[2rem] p-8 text-sm text-textmuted">Refreshing upcoming events...</div>
      ) : events.length === 0 ? (
        <div className="glass-panel rounded-[2rem] p-10 text-center">
          <h2 className="text-3xl font-bold text-white">No upcoming events found</h2>
          <p className="mt-3 text-sm leading-6 text-textmuted">
            Try another keyword or category to see more results.
          </p>
        </div>
      ) : (
